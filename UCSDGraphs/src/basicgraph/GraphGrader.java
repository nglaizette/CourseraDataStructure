...
/**
 * @author UCSD MOOC Development Team Grader for Module 1, Part 2
 */
public class GraphGrader {
	 ...
	 * @param corr
	 *            A list containing the correct answer
	 */
	 ...
	 *            The graph number
	 * @param file
	 *            The file to read the correct answer from
	 * @param desc
	 *            A description of the graph
	 * @param start
	 *            The node to start from
	 ...
	 * 
	 * @param result
	 *            The list with the user's result
	 * @param corr
	 *            The list with the correct answer
	 */
	public void judge(List<Integer> result, List<Integer> corr) {
		...
		if (result == null) {
			...
			feedback += "FAILED. Expected " + printList(corr) + ", got "
					+ printList(result) + ". ";
	...
	public void run() {
		...
		try {
			...
			runSpecialTest(7, "ucsd.map",
					"UCSD MAP: Intersections around UCSD", 3, correctAns,
					"road");

			correctAns = readCorrect("routesUA.dat.twoaway");
			runSpecialTest(8, "routesUA.dat",
					"AIRLINE MAP: Airplane routes around the world", 6,
					correctAns, "air");

			if (correct == TESTS)
				feedback = "All tests passed. Great job!" + feedback;
			else
				feedback = "Some tests failed. Check your code for errors, then try again:"
						+ feedback;
