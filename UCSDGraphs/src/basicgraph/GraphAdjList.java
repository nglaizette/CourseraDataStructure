...
public class GraphAdjList extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer> toto = getNeighbors(v);
		for (Integer integer : toto) {
			List<Integer> tutu = getNeighbors(integer.intValue());
			for (Integer integer2 : tutu) {
				// if (!res.contains(integer2)) {
				res.add(integer2);
				// }
			}
		}
